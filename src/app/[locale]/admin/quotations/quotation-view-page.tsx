...
// import { ArrowLeft, Download, FileText, Loader2, Calendar, User, Printer, Share2, Replace, Phone, Mail } from "lucide-react";
...
//                         <Label htmlFor="party-sig-toggle" className="text-[9px] md:text-[10px] uppercase font-bold text-muted-foreground whitespace-nowrap cursor-pointer">
//                             Party Signature
//                         </Label>
//                         <Switch 
//                             id="party-sig-toggle" 
...
//                 {/* Paper Header */}
//                 <div className="p-6 border-b pb-6">
...
//                                 {company.name}
//                             </h1>
//                             <p className="text-xs text-muted-foreground whitespace-pre-line leading-relaxed max-w-md mx-auto">
//                                 {company.address}
//                             </p>
//                             {(company.phone || company.email) && (
//                                 <div className="flex flex-wrap justify-center gap-x-6 gap-y-1 text-[10px] text-slate-500 mt-1">
//                                     {company.phone && (
//                                         <span className="whitespace-nowrap">
//                                             <Phone className="h-3.5 w-3.5 opacity-70" style={{ display: 'inline-block', verticalAlign: 'middle', marginRight: '4px', marginBottom: '2px' }} />
//                                             <span className="tabular-nums" style={{ display: 'inline-block', verticalAlign: 'middle' }}>{company.phone}</span>
//                                         </span>
//                                     )}
//                                     {company.email && (
//                                         <span className="whitespace-nowrap">
//                                             <Mail className="h-3.5 w-3.5 opacity-70" style={{ display: 'inline-block', verticalAlign: 'middle', marginRight: '4px', marginBottom: '2px' }} />
//                                             <span className="lowercase" style={{ display: 'inline-block', verticalAlign: 'middle' }}>{company.email}</span>
//                                         </span>
//                                     )}
//                                 </div>
//                             )}
...
//                                 <p className="font-bold text-slate-900">{quotation.party_details?.name || quotation.party_name}</p>
//                                 {quotation.party_details?.company_name && (
//                                     <p className="text-slate-700 font-medium">{quotation.party_details.company_name}</p>
//                                 )}
//                                 {quotation.party_details?.address && (
//                                     <p className="text-slate-600 text-[10px] leading-tight italic">{quotation.party_details.address}</p>
//                                 )}
//                                 {quotation.party_details?.phone && (
//                                     <p className="text-slate-600">{quotation.party_details.phone}</p>
//                                 )}
//                                 {quotation.party_details?.email && (
//                                     <p className="text-slate-600">{quotation.party_details.email}</p>
//                                 )}
//                             </div>
//                         </div>
//                         <div className="space-y-1 text-right text-sm">
//                             <div className="flex justify-end gap-4">
//                                 <span className="text-slate-500">{tInvoice("quotation_number") || "No:"}</span>
//                                 <span className="font-bold">{quotation.quotation_no || `#${quotation._id?.slice(-6).toUpperCase()}`}</span>
//                             </div>
//                             <div className="flex justify-end gap-4">
//                                 <span className="text-slate-500">{tInvoice("date") || "Date"}:</span>
//                                 <span className="font-bold">{quotation.created_at ? new Date(quotation.created_at).toLocaleDateString() : "N/A"}</span>
//                             </div>
//                             {quotation.validity_date && (
//                                 <div className="flex justify-end gap-4 text-slate-500">
//                                     <span className="font-medium">{tInvoice("valid_until") || "Valid Until"}:</span>
//                                     <span className="font-bold">{new Date(quotation.validity_date).toLocaleDateString()}</span>
//                                 </div>
//                             )}
//                         </div>
//                     </div>
...
//                                     <th className="text-left py-2 px-4">{tInvoice("item") || "Item"}</th>
//                                     <th className="text-right py-2 px-4">{tInvoice("sellPrice") || "Price"}</th>
//                                     <th className="text-right py-2 px-4">{tInvoice("qty") || "Qty"}</th>
//                                     <th className="text-right py-2 px-4">{tInvoice("uom") || "UOM"}</th>
//                                     <th className="text-right py-2 px-4">{tInvoice("total") || "Total"}</th>
//                                 </tr>
//                             </thead>
...
//                                                     <div className="text-xs text-muted-foreground mt-0.5 leading-relaxed">{item.product_description}</div>
//                                                 )}
//                                             </div>
//                                         </td>
//                                         <td className="text-right py-2 px-4 text-slate-600">{formatCurrencyString(item.unit_price || item.sell_price)}</td>
//                                         <td className="text-right py-2 px-4 text-slate-700">{item.quantity}</td>
//                                         <td className="text-right py-2 px-4 text-slate-600">{item.unit_of_measurement || "-"}</td>
//                                         <td className="text-right py-2 px-4 font-bold text-slate-900">{formatCurrencyString(item.amount)}</td>
//                                     </tr>
//                                 ))}
//                             </tbody>
//                         </table>
...
//                                 <span>{tInvoice("subTotal") || "Sub Total"}:</span>
//                                 <span>{formatCurrencyString(quotation.items?.reduce((sum: number, item: any) => sum + item.amount, 0) || 0)}</span>
//                             </div>
                            
//                             {quotation.discount > 0 && (
//                                 <div className="flex justify-between text-destructive">
//                                     <span>{tInvoice("discount") || "Discount"} ({quotation.discount}{quotation.discount_type === "percentage" ? "%" : ""}):</span>
...
//                             {quotation.tax > 0 && (
//                                 <div className="flex justify-between text-slate-600">
//                                     <span>{tInvoice("tax") || "Tax"} ({quotation.tax}{quotation.tax_type === "percentage" ? "%" : ""}):</span>
//                                     <span>+{formatCurrencyString(
...
//                             <div className="flex justify-between font-bold text-lg border-t pt-2 text-slate-900">
//                                 <span>Grand Total:</span>
//                                 <span>{formatCurrencyString(quotation.total_amount || 0)}</span>
//                             </div>
//                         </div>
//                     </div>

//                     {quotation.notes && (
//                         <div className="mt-8 text-sm text-muted-foreground border-l-2 border-primary/20 pl-4 py-2 bg-slate-50/30 rounded-r-lg max-w-md">
//                             <p className="font-bold text-slate-800 mb-1 text-[10px] uppercase tracking-wider">
...
//                                     />
//                                 )}
//                                 <div className="w-40 border-t border-slate-300" />
...
//                                 <div className="h-16 w-48 border-2 border-dashed border-slate-200 flex items-center justify-center rounded-lg">
//                                     <span className="text-[10px] text-slate-300 uppercase tracking-widest font-bold">Sign Here</span>
//                                 </div>
//                                 <div className="w-40 border-t border-slate-300" />
//                                 <span className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold">
