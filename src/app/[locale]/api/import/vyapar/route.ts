...
async function detectTables(db: Database) {
  ...
  for (const table of tables) {
    ...
    if (!result.partyTable && (
      (colSet.has('full_name') && (colSet.has('phone_number') || colSet.has('email'))) ||
      (colSet.has('name') && (colSet.has('phone') || colSet.has('email')))
    )) {
      result.partyTable = table;
      console.log(`✅ Party table: ${table}`);
      continue;
    }
    ...
    if (!result.productTable && (
      ...
      result.productTable = table;
      console.log(`✅ Product table: ${table}`);
      continue;
    }
    ...
    if (!result.paymentMethodTable && (
      ...
      result.paymentMethodTable = table;
      console.log(`✅ Payment method table: ${table}`);
      continue;
    }

    if (colSet.has('quantity') && (colSet.has('item_id') || colSet.has('lineitem_txn_id'))) {
      result.lineItemTables.push(table);
      console.log(`✅ Line item table: ${table}`);
      continue;
    }

    if ((colSet.has('txn_date') || colSet.has('date')) && 
        (colSet.has('txn_cash_amount') || colSet.has('txn_balance_amount')) &&
        colSet.has('txn_name_id')) {
      result.transactionTables.push(table);
      console.log(`✅ Transaction table: ${table}`);
      continue;
    }
  }
...
async function importParties(db: Database, table: string, userId: string, partiesCollection: any, summary: any) {
  const rows = await readAll(db, table);
  console.log(`👥 Importing ${rows.length} parties from ${table}...`);
  
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    const name = firstValue(row, ["full_name", "name"]);
    if (!name) continue;

    const externalId = `vyapar_party_${row.name_id ?? i}`;
    const phone = firstValue(row, ["phone_number", "phone"]);
    const email = firstValue(row, ["email"]);
    const openingBalance = firstNumber(row, ["amount"], 0);
    ...
    if (!existing) {
      ...
      const result = await partiesCollection.insertOne({
        ...
        phone: phone,
        company_name: "",
        company_address: firstValue(row, ["address"], ""),
        opening_balance: openingBalance,
        status: "active",
        is_delete: 0,
        created_at: now,
      ...
      if (result.insertedId && openingBalance !== 0) {
        await seedCustomerOpeningBalance(userId, result.insertedId.toString(), openingBalance, now);
      }
      summary.partiesImported++;
    } else {
      summary.partiesImported++;
    }
  }
...
async function importProducts(db: Database, table: string, userId: string, productsCollection: any, summary: any) {
  const rows = await readAll(db, table);
  console.log(`📦 Importing ${rows.length} products from ${table}...`);
  
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    const name = firstValue(row, ["item_name", "name"]);
    if (!name) continue;

    const salePrice = firstNumber(row, ["item_sale_unit_price"], 0);
    const purchasePrice = firstNumber(row, ["item_purchase_unit_price"], 0);
    const quantity = firstNumber(row, ["item_stock_quantity"], 0);
    ...
    if (!existing) {
      await productsCollection.insertOne({
        user_id: toObjectId(userId),
        name: name,
        type: firstValue(row, ["item_type"], "goods"),
        category: firstValue(row, ["category_id"], ""),
        description: firstValue(row, ["item_description"], ""),
        unit_of_measurement: "",
        sale_price: salePrice,
        purchase_price: purchasePrice,
        quantity: quantity,
        ...
        stock: quantity,
        damaged_quantity: 0,
        barcode: firstValue(row, ["item_code"], ""),
        created_at: new Date(),
        updated_at: new Date(),
      });
...
async function importPaymentMethods(db: Database, table: string, userId: string, paymentMethodCollection: any, summary: any) {
  const rows = await readAll(db, table);
  console.log(`💳 Importing ${rows.length} payment methods from ${table}...`);
  
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    const bankName = firstValue(row, ["paymenttype_name", "name"]);
    if (!bankName) continue;

    const existing = await paymentMethodCollection.findOne({
      user_id: toObjectId(userId),
      bank_name: bankName,
    });

    if (!existing) {
      await paymentMethodCollection.insertOne({
        user_id: toObjectId(userId),
        bank_name: bankName,
        account_number: firstValue(row, ["paymenttype_accountnumber", "account_number"], ""),
        ifsc_code: firstValue(row, ["pt_bank_ifsc_code", "ifsc_code"], ""),
        branch_name: "",
        type: firstValue(row, ["paymenttype_type"], ""),
        bank_details: "",
        created_at: new Date(),
        updated_at: new Date(),
      });
      summary.paymentMethodsImported++;
    } else {
      summary.paymentMethodsImported++;
    }
  }
...
async function importTransactions(
  db: Database, 
  txnTable: string, 
  lineItemTables: string[], 
  userId: string, 
  partiesCollection: any,
  productsCollection: any,
  ordersCollection: any,
  purchaseBillsCollection: any,
  partyTransactionCollection: any,
  ...
  // Get all parties for lookup
  const parties = await partiesCollection.find({ user_id: toObjectId(userId) }).toArray();
  const partyByName = new Map();
  const partyById = new Map();
  for (const p of parties) {
    partyByName.set(p.name.toLowerCase(), p);
    partyById.set(p._id.toString(), p);
  }

  // Get all products for lookup
  const products = await productsCollection.find({ user_id: toObjectId(userId) }).toArray();
  ...
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    const txnType = firstValue(row, ["txn_type"], "");
    const txnTypeLower = txnType.toLowerCase();
    
    // Determine transaction kind
    let kind: "sale" | "purchase" | "payment_in" | "payment_out" | "expense" = "expense";
    if (txnTypeLower.includes("sale") || txnTypeLower.includes("sell")) kind = "sale";
    else if (txnTypeLower.includes("purchase") || txnTypeLower.includes("buy")) kind = "purchase";
    else if (txnTypeLower.includes("receipt") || txnTypeLower.includes("received")) kind = "payment_in";
    else if (txnTypeLower.includes("payment") || txnTypeLower.includes("paid")) kind = "payment_out";
    
    const date = parseDateValue(row.txn_date ?? row.date);
    const partyRef = row.txn_name_id;
    let partyId = null;
    let partyDoc = null;
    ...
    const totalAmount = firstNumber(row, ["txn_balance_amount"], 0);
    const paidAmount = firstNumber(row, ["txn_cash_amount"], 0);
    const amount = totalAmount > 0 ? totalAmount : paidAmount;
    const invoiceNo = firstValue(row, ["txn_ref_number_char"], "");
    
    // Get line items
    let lineRows: AnyRow[] = [];
    const txnId = row.txn_id;
    if (txnId) {
      for (const lineTable of lineItemTables) {
        const lineItems = await readAll(db, lineTable);
        const filtered = lineItems.filter(l => String(l.lineitem_txn_id) === String(txnId));
    ...
    for (const line of lineRows) {
      ...
      orderItems.push({
        ...
        quantityType: "prime",
        price: price,
        discount: firstNumber(line, ["lineitem_discount_amount"], 0),
        discountType: "value",
        unit_of_measurement: "",
      });
    }
    ...
    // === SALE ===
    if (kind === "sale" && orderItems.length > 0 && partyId) {
      ...
      if (!existingOrder) {
        const orderResult = await ordersCollection.insertOne({
          ...
          due_date: null,
          charges: [],
          overallDiscount: firstNumber(row, ["txn_discount_amount"], 0),
          shippingCharges: 0,
          items: orderItems,
          payment: paidAmount > 0 ? {
            method: "cash",
        ...
        if (orderResult.insertedId) {
          await appendCustomerLedgerEntry({
            userId: userId,
            customerId: partyId,
            eventKey: `vyapar_sale_${orderResult.insertedId.toString()}`,
            eventType: "order_debit",
            eventSource: "order",
            eventSourceId: orderResult.insertedId.toString(),
            amountDelta: amount,
            effectiveAt: date,
            metadata: { source: "vyapar_import", invoice_no: invoiceNo },
          });
          ...
          if (paidAmount > 0) {
            await appendCustomerLedgerEntry({
              userId: userId,
              customerId: partyId,
              eventKey: `vyapar_sale_payment_${orderResult.insertedId.toString()}`,
              eventType: "order_payment_credit",
              eventSource: "order",
              eventSourceId: orderResult.insertedId.toString(),
              amountDelta: -paidAmount,
    ...
    // === PURCHASE ===
    if (kind === "purchase" && partyId) {
      const existingBill = await purchaseBillsCollection.findOne({
        user_id: toObjectId(userId),
        party_id: toObjectId(partyId),
        total_amount: amount,
      });
      ...
      if (!existingBill) {
        const items = lineRows.map(line => ({
          product_id: null,
          product_name: firstValue(line, ["item_name"], ""),
          product_description: "",
          quantity: firstNumber(line, ["quantity"], 0),
          cost_price: firstNumber(line, ["priceperunit"], 0),
          amount: firstNumber(line, ["total_amount"], 0),
        }));
        
        await purchaseBillsCollection.insertOne({
          user_id: toObjectId(userId),
          party_id: toObjectId(partyId),
          party_name: partyDoc?.name || "",
          items: items,
          discount: firstNumber(row, ["txn_discount_amount"], 0),
          discount_type: "fixed",
          tax: firstNumber(row, ["txn_tax_amount"], 0),
          tax_type: "fixed",
          total_amount: amount,
          paid_amount: paidAmount,
          balance_due: amount - paidAmount,
          ...
          description: firstValue(row, ["txn_description"], ""),
          created_at: new Date(),
          updated_at: new Date(),
        });
        summary.purchaseBillsImported++;
    ...
    // === PAYMENT IN / OUT ===
    if ((kind === "payment_in" || kind === "payment_out") && partyId && amount > 0) {
      const existingTxn = await partyTransactionCollection.findOne({
        user_id: toObjectId(userId),
        customer_id: toObjectId(partyId),
      ...
      if (!existingTxn) {
        await partyTransactionCollection.insertOne({
          user_id: toObjectId(userId),
          customer_id: toObjectId(partyId),
          payment_amount: amount,
          payment_method_id: "cash",
          date: date,
          type: kind === "payment_in" ? "payment-in" : "payment-out",
          created_at: new Date(),
          updated_at: new Date(),
        });
        ...
        await appendCustomerLedgerEntry({
          userId: userId,
          customerId: partyId,
          eventKey: `vyapar_payment_${Date.now()}_${i}`,
          eventType: kind === "payment_in" ? "payment_in_credit" : "payment_out_debit",
          eventSource: "party_transaction",
          eventSourceId: `${Date.now()}`,
          amountDelta: kind === "payment_in" ? -amount : amount,
          effectiveAt: date,
          metadata: { source: "vyapar_import" },
        });
        summary.partyTransactionsImported++;
      } else {
        summary.partyTransactionsImported++;
    ...
    if (kind === "expense" && amount > 0) {
      ...
      if (!existingExpense) {
        await expensesCollection.insertOne({
          user_id: toObjectId(userId),
          expense_number: invoiceNo || `EXP_${i}`,
          date: date,
          category: firstValue(row, ["txn_category_id"], "Misc"),
          item_name: firstValue(row, ["txn_description"], "Expense"),
          description: firstValue(row, ["txn_description"], ""),
          qty: 1,
          rate: amount,
          amount: amount,
    ...
    if (amount > 0) {
      ...
      if (!existingExpense) {
        await expensesCollection.insertOne({
          ...
          date: date,
          category: "Unclassified",
          item_name: partyDoc?.name || "Unknown",
          description: firstValue(row, ["txn_description"], ""),
          qty: 1,
          rate: amount,
          amount: amount,
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (detected.partyTable) {
      await importParties(db, detected.partyTable, user.id, partiesCollection, summary);
      // Also sync to CUSTOMERS collection if needed (parties and customers are same)
      const parties = await partiesCollection.find({ user_id: toObjectId(user.id) }).toArray();
    ...
    if (detected.productTable) {
      await importProducts(db, detected.productTable, user.id, productsCollection, summary);
    }

    if (detected.paymentMethodTable) {
      await importPaymentMethods(db, detected.paymentMethodTable, user.id, paymentMethodCollection, summary);
    }

    if (detected.transactionTables.length > 0) {
      for (const txnTable of detected.transactionTables) {
        await importTransactions(
          db, txnTable, detected.lineItemTables, user.id,
          partiesCollection, productsCollection, ordersCollection,
          purchaseBillsCollection, partyTransactionCollection, expensesCollection,
          summary
        );
      }
    }
    ...
    return NextResponse.json({
      success: true,
      message: "Vyapar backup imported successfully into all collections",
      summary,
    });
