...
export async function GET(request: Request) {
  ...
  try {
    ...
    // If fetching single bill, return just that bill, not an array
  ...
  try {
    ...
    const enrichedItems = await Promise.all(
      items.map(async (item: any) => {
        ...
        // If we have a product_id, fetch additional details
    ...
    // Record ledger entry for the purchase bill
    if (totalAmount !== 0) {
      try {
        await appendPartyLedgerEntry({
          ...
          amountDelta: totalAmount, // Purchase increases party debt (debit)
          effectiveAt: now,
          metadata: {
        ...
        // Continue even if ledger entry fails - the bill is still created
      }
    }
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const result = await purchaseBillsCollection.updateOne(
      { _id: billObjId, user_id: userObjId },
      {
        $set: {
